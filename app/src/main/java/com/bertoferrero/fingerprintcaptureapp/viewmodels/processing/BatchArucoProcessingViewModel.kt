...
class BatchArucoProcessingViewModel(
    ...
    private suspend fun processImagesIndividually(
        ...
        val csvHeader = "filename;marker_id;x;y;z;ransac_threshold;filter_type;is_global_position;marker_count;image_markers_detected;ransac_population;is_ransac_excluded;detection_profile\n"
        ...
        for ((index, imageFile) in imageFiles.withIndex()) {
            ...
            try {
                    ...
                    if (imageInfo.success) {
                        ...
                        if (result.detectedPositions.isNotEmpty()) {
                            for (position in result.detectedPositions) {
                                ...
                                val csvLine = "${imageInfo.fileName};${position.markerId};${position.x.toCsvDecimal()};${position.y.toCsvDecimal()};${position.z.toCsvDecimal()};${position.ransacThreshold.toCsvDecimal()};${arithmeticFilterType.name};${position.isGlobalPosition};${position.markerCount};${imageInfo.markerCount};$ransacPopulation;${position.ransacExcluded};${detectionProfile.name}\n"
                                outputStream.write(csvLine.toByteArray())
                            }
                        } else {
                            // Imagen procesada correctamente pero sin detecciones
                            val noDetectionLine = "${imageInfo.fileName};NO_DETECTION;;;;;;;${imageInfo.markerCount};0;[];;${detectionProfile.name}\n"
                            outputStream.write(noDetectionLine.toByteArray())
                        }
                    } else {
                        // Error específico de esta imagen
                        val errorLine = "${imageInfo.fileName};ERROR;;;;;${imageInfo.error ?: "Image processing error"};;;${imageInfo.markerCount};;;${detectionProfile.name}\n"
                ...
                if (!result.success && result.error != null) {
                    val globalErrorLine = "${imageFile.name};GLOBAL_ERROR;;;;;${result.error};;;0;;;${detectionProfile.name}\n"
                ...
                val errorLine = "${imageFile.name};EXCEPTION;;;;;${e.message};;;0;;;${detectionProfile.name}\n"
                outputStream.write(errorLine.toByteArray())
            }
        }
    }
    ...
    private suspend fun processImagesUnified(
        ...
        val csvHeader = "image_name;marker_id;x;y;z;ransac_threshold;filter_type;is_global_position;marker_count;total_images_processed;successful_images;ransac_population;is_ransac_excluded;detection_profile\n"
        outputStream.write(csvHeader.toByteArray())
        ...
        try {
            ...
            for (position in individualPositions) {
                ...
                val csvLine = "$imageName;${position.markerId};${position.x.toCsvDecimal()};${position.y.toCsvDecimal()};${position.z.toCsvDecimal()};;;${position.isGlobalPosition};${position.markerCount};;$successfulImages;;${position.ransacExcluded};${detectionProfile.name}\n"
                outputStream.write(csvLine.toByteArray())
            }

            // Escribir información de imágenes con errores
            for (imageInfo in result.processedImages.filter { !it.success }) {
                val errorLine = "${imageInfo.fileName};ERROR;;;;;${imageInfo.error ?: "Image processing error"};;;${imageInfo.markerCount};$totalImagesProcessed;$successfulImages;0;${detectionProfile.name}\n"
                outputStream.write(errorLine.toByteArray())
            }

            // Si hubo error global
            if (!result.success && result.error != null) {
                val globalErrorLine = ";GLOBAL_ERROR;;;;;${result.error};;;0;$totalImagesProcessed;$successfulImages;;${detectionProfile.name}\n"
                outputStream.write(globalErrorLine.toByteArray())
            }

            // Si no se detectaron posiciones
            if (result.detectedPositions.isEmpty() && result.success) {
                val noDetectionLine = ";NO_DETECTION;;;;;;;0;$totalImagesProcessed;$successfulImages;;;${detectionProfile.name}\n"
                outputStream.write(noDetectionLine.toByteArray())
            }
            ...
            val errorLine = ";EXCEPTION;;;;;${e.message};;;0;${imageFiles.size};0;0;${detectionProfile.name}\n"
            outputStream.write(errorLine.toByteArray())
        }
    ...
    /**
     * Genera un JSON con la información de los marcadores RANSAC.
     */
    private fun generateRansacMarkersJson(ransacResult: List<com.bertoferrero.fingerprintcaptureapp.lib.positioning.PositionFromMarker>?): String {
        if (ransacResult.isNullOrEmpty()) return "[]"
        
        val markersInfo = ransacResult.map { marker ->
            mapOf(
                "marker_id" to marker.markerId,
                "source_image" to (marker.sourceIdentifier ?: "unknown")
            )
        }
        
        // Convertir a JSON simple (sin usar librerías externas)
        return markersInfo.joinToString(
            prefix = "[",
            postfix = "]",
            separator = ","
        ) { marker ->
            "{" + marker.entries.joinToString(",") { (key, value) ->
                when (value) {
                    is String -> "\"$key\":\"$value\""
                    is Number -> "\"$key\":$value"
                    else -> "\"$key\":\"$value\""
